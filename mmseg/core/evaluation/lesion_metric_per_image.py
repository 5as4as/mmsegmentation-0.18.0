...
def sigmoid_metrics(results, gt_seg_maps, num_classes, compute_aupr, use_sigmoid):
    ...
    total_iou = np.zeros((len(threshs), num_classes))
    total_dice = np.zeros((len(threshs), num_classes))
    total_se = np.zeros((len(threshs), num_classes))
    total_sp = np.zeros((len(threshs), num_classes))
    total_mae = np.zeros((len(threshs), num_classes))
    ...
    for i in range(num_imgs):
        ...
        for k, thresh in enumerate(threshs):
            ...
            total_iou[k][label == 0] += 1
            total_dice[k][label == 0] += 1
            total_sp[k][p == 0] += 1
            total_se[k][label == 0] += 1

            total_iou[k][label > 0] += tp[label > 0] / (p[label > 0] + fn[label > 0])
            total_dice[k][label > 0] += 2 * tp[label > 0] / (p[label > 0] + label[label > 0])
            total_sp[k][p > 0] += tp[p > 0] / p[p > 0]
            total_se[k][label > 0] += tp[label > 0] / label[label > 0]
    ...
    if len(threshs) > 1:
        ...
        index = 0
    iou = total_iou[index]
    dice = total_dice[index]
    se = total_se[index]
    sp = total_sp[index]
    mae = total_mae[index]

    return iou / num_imgs, dice / num_imgs, se / num_imgs, sp / num_imgs, aupr / num_imgs, mae / num_imgs
...
def lesion_metrics_per_image(results, gt_seg_maps, num_classes, ignore_index=None, nan_to_num=None):
    """
    ...
    if isinstance(results[0], tuple):
        use_sigmoid = results[0][0].shape[0] == 4
