...
class MultiTaskGAN(BaseGAN):
    ...
    def _get_aux_disc_loss(self, outputs_dict):
        ...
        losses_dict = {}
        # gan loss
        losses_dict['loss_disc_aux_target_1'] = 0.5 * self.gan_loss(
            outputs_dict['disc_aux_pred_1'][1], outputs_dict['label'][1])
        losses_dict['loss_disc_aux_source_1'] = 0.5 * self.gan_loss(
            outputs_dict['disc_aux_pred_1'][0], outputs_dict['label'][0])

        losses_dict['loss_disc_aux_target_2'] = 0.5 * self.gan_loss(
            outputs_dict['disc_aux_pred_2'][1], outputs_dict['label'][1])
        losses_dict['loss_disc_aux_source_2'] = 0.5 * self.gan_loss(
            outputs_dict['disc_aux_pred_2'][0], outputs_dict['label'][0])
        # disc auxiliary loss
    ...
    def _get_disc_loss(self, outputs_dict):
        ...
        losses_dict = {}
        # gan loss
        losses_dict['loss_disc_target_1'] = 0.5 * self.gan_loss(
            outputs_dict['disc_pred_1'][1], outputs_dict['label'][1])
        losses_dict['loss_disc_source_1'] = 0.5 * self.gan_loss(
            outputs_dict['disc_pred_1'][0], outputs_dict['label'][0])

        losses_dict['loss_disc_target_2'] = 0.5 * self.gan_loss(
            outputs_dict['disc_pred_2'][1], outputs_dict['label'][1])
        losses_dict['loss_disc_source_2'] = 0.5 * self.gan_loss(
            outputs_dict['disc_pred_2'][0], outputs_dict['label'][0])
    ...
    def _get_gen_loss(self, outputs_dict):
        ...
        losses_dict = {}
        # gan loss
        losses_dict['loss_gen_target_1'] = self.ODOC_lamda * self.gan_loss(
            outputs_dict['disc_pred_target'][0], outputs_dict['label_target'][0])
        losses_dict['loss_gen_target_2'] = self.lesion_lamda * self.gan_loss(
            outputs_dict['disc_pred_target'][1], outputs_dict['label_target'][1])
        if outputs_dict['with_auxiliary_head']:
            losses_dict['loss_gen_target_aux_1'] = self.ODOC_lamda_aux * self.gan_loss(
                outputs_dict['disc_aux_pred_target'][0], outputs_dict['label_target'][0])
            losses_dict['loss_gen_target_aux_2'] = self.lesion_lamda_aux * self.gan_loss(
                outputs_dict['disc_aux_pred_target'][1], outputs_dict['label_target'][1])
    ...
    def train_step(self,
        ...
        label_2 = torch.zeros((B_2, 1, 512, 512), device=imgs[1].device)
        label_adv_1 = torch.ones((B_2, 1, 512, 512), device=imgs[0].device)
        label_adv_2 = torch.ones((B_1, 1, 512, 512), device=imgs[1].device)
        ...
        if with_auxiliary_head:
            for i in range(len(target_seg)):
                img_num = target_seg[i].shape[0] // 2
                auxiliary_target_seg = target_seg[i][img_num:]
                target_seg[i] = target_seg[i][:img_num]
                disc_auxi_pred_target_g.append(self.auxiliary_discriminator[i](auxiliary_target_seg))

        for i in range(len(target_seg)):
            disc_pred_target_g.append(self.discriminator[i](target_seg[i]))
        ...
        if with_auxiliary_head:
            for i in range(len(pred_1)):
                n1 = pred_1[i].shape[0] // 2
                aux_pred = pred_1[i][n1:]
                pred_1[i] = pred_1[i][:n1]
                disc_aux_pred_1.append(self.auxiliary_discriminator[0](aux_pred))
            for i in range(len(pred_2)):
                n1 = pred_2[i].shape[0] // 2
                aux_pred = pred_2[i][n1:]
                pred_2[i] = pred_2[i][:n1]
                disc_aux_pred_2.append(self.auxiliary_discriminator[1](aux_pred))
        # disc pred for fake imgs and real_imgs
        disc_pred_1 = []
        disc_pred_2 = []
        for i in range(len(pred_1)):
            disc_pred_1.append(self.discriminator[0](pred_1[i]))
        for i in range(len(pred_2)):
            disc_pred_2.append(self.discriminator[1](pred_2[i]))
        # get data dict to compute losses for disc
        data_dict_ = dict(
            disc_pred_1=disc_pred_1,
            disc_pred_2=disc_pred_2,
            disc_aux_pred_1=disc_aux_pred_1,
            disc_aux_pred_2=disc_aux_pred_2,
            label=label,
            with_auxiliary_head=with_auxiliary_head)
