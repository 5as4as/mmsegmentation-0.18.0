        ...
        for i, seg in enumerate(seg_logit):
            if i == 0:
                if isinstance(seg, list):
                    tmp = []
                    for sseg in seg:
                        tmp.append(F.softmax(sseg, dim=1))
                    output.append(tmp)
                else:
                    output.append(F.softmax(seg, dim=1))
            elif i == 1:
                if isinstance(seg, list):
                    tmp = []
                    for sseg in seg:
        ...
        for i, seg in enumerate(seg_logit):
            if i == 0:
                if isinstance(seg, list):
                    tmp = []
                    for sseg in seg:
                        seg_pred = sseg.argmax(dim=1)
                        seg_pred = seg_pred.squeeze(0).cpu().numpy()
                        tmp.append(seg_pred)
                    seg_logit[i] = tmp
                else:
                    seg_pred = seg.argmax(dim=1)
                    seg_pred = seg_pred.squeeze(0).cpu().numpy()
                ...
                if isinstance(seg, list):
                    tmp = []
                    for sseg in seg:
                        sseg = sseg.squeeze(0).cpu().numpy()
                        label = sseg.argmax(axis=0) + 1
                        mask = (sseg < 0.5).all(axis=0)
                        label[mask] = 0
                        tmp.append(label)
                    seg_logit[i] = tmp
                else:
                    seg = seg.squeeze(0).cpu().numpy()
                    label = seg.argmax(axis=0) + 1
                    mask = (seg < 0.5).all(axis=0)
                    label[mask] = 0
                    seg_logit[i] = label
        if test_mode:
