...
class MultiGenerator(Encoder_Multi_Decoder):
    ...
    def forward(self, img, img_meta, with_auxiliary_head=False, **kwargs):
        seg_logit = self.encode_decode(img, img_meta, with_auxiliary_head=with_auxiliary_head)
        output = []
        if isinstance(seg_logit[0], list):
            output.append(F.softmax(seg_logit[0][0], dim=1))
        else:
            output.append(F.softmax(seg_logit[0], dim=1))
        if isinstance(seg_logit[1], list):
            output.append(torch.sigmoid(seg_logit[1][0]))
        else:
            output.append(torch.sigmoid(seg_logit[1]))
        return seg_logit, output


@MODULES.register_module()
class MultiTaskGenerator(Encoder_Decoder_Multi_task):
    ...
    def forward(self, img, img_meta, with_auxiliary_head=False, **kwargs):
        seg_logit = self.encode_decode(img, img_meta, with_auxiliary_head=with_auxiliary_head)
        seg_logits = []
        seg_logits.append(torch.cat(seg_logit[0][:2], seg_logit[2][:2]))
        seg_logits.append(torch.cat(seg_logit[1][2:], seg_logit[3][2:]))
        output = []
