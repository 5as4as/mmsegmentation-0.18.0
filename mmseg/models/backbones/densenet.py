...
class DenseNet(BaseModule):
    ...
    def __init__(self,
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(
                ...
                padding=3,
                bias=False),
            build_norm_layer(norm_cfg, self.init_channels)[1], build_activation_layer(act_cfg),
            ...
            for k, v in _state_dict.items():
                if k.startswith('features.'):
                    ...
                    if k.startswith('conv0.'):
                        state_dict['stem.0.weight'] = v
                    elif k.startswith('norm0.'):
                        state_dict['stem.1.'+k[6:]] = v
                    elif k.startswith('norm5.'):
                        state_dict['transitions.3.0'+k[6:]] = v
                    elif k.startswith('transition'):
                        state_dict['transition.'+str(int(k[10]) - 1)+k[11:]] = v
                    else:
                        tmp = k.split('.')
                        key = 'stage.' + str(int(k[10]) - 1) + '.block.' + str(int(k[22]) - 1) + '.' + tmp[-3] + tmp[-2] + '.' + tmp[-1]
                        state_dict[key] = v
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            x = self.stages[i](x)
            if i in self.out_indices and i != self.num_stages - 1:
                outs.append(x)
