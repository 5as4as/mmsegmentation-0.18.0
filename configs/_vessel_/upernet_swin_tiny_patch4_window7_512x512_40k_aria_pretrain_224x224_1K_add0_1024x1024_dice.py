...
model = dict(
    ...
    backbone=dict(
        ...
        drop_path_rate=0.3,
        patch_norm=True),
    decode_head=dict(in_channels=[96, 192, 384, 768], num_classes=2, loss_decode=dict(type='DiceLoss', smooth=1e-5, loss_weight=1.0)),
    auxiliary_head=dict(in_channels=384, num_classes=2, loss_decode=dict(type='DiceLoss', smooth=1e-5, loss_weight=0.4)))
